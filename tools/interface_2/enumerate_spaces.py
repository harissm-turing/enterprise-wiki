...
class EnumerateSpaces(Tool):
    ...
    def invoke(
        ...
        if is_approved is not None:
            result = [
                space for space in result if space.get("is_approved") == is_approved
            ]
    ...
    def get_info() -> Dict[str, Any]:
        return {
            "type": "function",
            "function": {
                "name": "enumerate_spaces",
                "description": "Enumerate spaces, optionally filtered by type or approval status",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "space_type": {
