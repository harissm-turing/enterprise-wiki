...
class UpdateSpace(Tool):
    @staticmethod
    def invoke(data: Dict[str, Any], space_id: str, updated_by: str, 
               name: Optional[str] = None, key: Optional[str] = None, 
               space_type: Optional[str] = None, is_approved: Optional[bool] = None) -> str:
        ...
        if key is not None:
            if any(s.get("key") == key.upper() and s_id != space_id 
    ...
    def get_info() -> Dict[str, Any]:
        return {
            ...
            "function": {
                "name": "update_space",
                "description": "Update an existing space in the wiki",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "space_id": {
